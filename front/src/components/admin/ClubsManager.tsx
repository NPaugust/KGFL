                ...
                <tr key={club.id} className="border-b border-white/10">
                  <td className="px-4 py-3">
                    {(club as any).logo_url ? (
                      <Image src={(club as any).logo_url.startsWith('http') ? (club as any).logo_url : `/${(club as any).logo_url}`} alt={club.name} width={48} height={48} className="w-12 h-12 rounded-lg object-contain bg-transparent border-2 border-white/20" />
                    ) : (
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex gap-4 pt-5 border-t border-white/10">
            ...
            <button type="button" onClick={() => { setIsModalOpen(false); setEditingClub(null); }} className="btn btn-outline flex-1">
              Отмена
            </button>
          </div>
        </form>
