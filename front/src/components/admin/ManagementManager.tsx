...
export function ManagementManager() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingManager) {
        await mutate(API_ENDPOINTS.MANAGEMENT_DETAIL(editingManager.id), 'PUT', payload)
      } else {
            ...
            <tbody>
              {managersList.length > 0 ? managersList.map((manager) => (
                <tr key={manager.id} className="border-b border-white/10">
                  ...
                  <td className="px-4 py-3">{manager.position}</td>
      ...
      >
        <div className="p-6">
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Фото {editingManager ? '' : '*'}</label>
