...
def update_club_stats_after_match(sender, instance, created, **kwargs):
    ...
    """
    if not created and instance.season and instance.status in ['finished', 'live'] and instance.home_team and instance.away_team:
        # Обновляем статистику для завершенных матчей и матчей в прямом эфире
        from clubs.models import ClubSeason
